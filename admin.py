...
reasons_map = [
    ...
    {"name": "Flag off & Torch", "color": "c-flg"}, # Shortened name for UI fit
...
if not df_filt.empty:
    ...
    for r in reasons_map:
        # Check full name match (handle slight variations if needed)
        # Using simple match here based on previous list
        key = r["name"]
        if key == "Flag off & Torch": key = "Flag off & Torch bearers" # map back to DB value
        reason_counts[r["name"]] = grouped.get(key, 0)
else:
